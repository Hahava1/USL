...
def annotations_to_instances(annos, image_size):
    ...
    # [REMOVIDO] Já não carregamos 'dz' (ground-truth z)
...
class MeshRCNNMapper:
    ...
    def transform_annotations(self, annotation, transforms, image_size):
        ...
        # [MODIFICADO] Renomeado para 'layout_on' e removemos o processamento de 'dz'
        # O USL não usa supervisão 3D 'dz',
        # por isso não precisamos de a processar.
        if self.layout_on and "mesh" in annotation:
             # annotation["dz"] = self._process_dz(...) # Linha removida
             pass
        else:
    ...
    # [REMOVIDO] A função _process_dz inteira foi removida
    # O USL não usa esta supervisão 3D.
